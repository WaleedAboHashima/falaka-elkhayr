  ...
  if ((!username || !email || !password || !phone || !location, !government)) {
    ...
    try {
      ...
      if (sr) {
        res
          .status(409)
          .json({ message: "User or Sr with this info already exists." });
      } else {
        await User.create({
          username,
          email,
...
exports.GetCustomers = asnycHandler(async (req, res, next) => {
  const storeId = req.user.id;
  try {
...
exports.AddTerms = asnycHandler(async (req, res, next) => {
  const storeId = req.user.id;
  const { terms } = req.body;
  if (!terms) return next(new ApiError("Terms Required", 403));
  else {
    await Store.findByIdAndUpdate(storeId, { terms: terms }).then((store) => {
      if (store) {
        res.sendStatus(200);
      } else {
